 ...
 *
 * REV 2.1 @ 30.01.2018
 * 		--> params loading from input file
...
const int SIZE_A = 46; //bits
const int SIZE_K = 53; //bits
...
void writeResultToFile(const char* filePath, float duration,
	...
	char dur[100];
	char text[] = " Found %d prime numbers (%d bit) in %.3f "
						"seconds requiring %d number generations\n";
	int charsWritten = snprintf(dur, 100, text, primeNums, numBits,
	duration, numGen);
	if(charsWritten < 0) {
